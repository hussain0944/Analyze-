...
• `/enable_trading` - تفعيل التداول (للمشرفين)
• `/disable_trading` - إيقاف التداول (للمشرفين)
• `/trading_stats` - إحصائيات التداول
• `/advanced_trading` - حالة التداول المتقدم
• `/enable_advanced` - تفعيل التداول المتقدم (للمشرفين)
• `/disable_advanced` - إيقاف التداول المتقدم (للمشرفين)

🖼️ **تحليل الصور:**
• **أرسل صورة رسم بياني** - سيتم تحليلها تلقائياً
...
from bot_new_commands import (
    ...
    handle_approval_callback, add_user_by_id_command, search_telegram_user_command,
    auto_trading_status_command, enable_auto_trading_command, 
    disable_auto_trading_command, trading_stats_command
)
...
if __name__ == "__main__":
    ...
    app.add_handler(CommandHandler("add_user", add_user_by_id_command))
    app.add_handler(CommandHandler("search_telegram", search_telegram_user_command))
    ...
    # أوامر التداول التلقائي (للمشرفين)
    app.add_handler(CommandHandler("auto_trading", auto_trading_status_command))
    app.add_handler(CommandHandler("enable_trading", enable_auto_trading_command))
    app.add_handler(CommandHandler("disable_trading", disable_auto_trading_command))
    app.add_handler(CommandHandler("trading_stats", trading_stats_command))
    
    # أوامر التداول المتقدم (جديد)
    try:
        from bot_advanced_commands import (
            advanced_trading_command, enable_advanced_trading_command, 
            disable_advanced_trading_command, advanced_trading_stats_command,
            handle_chart_image, timeframes_command, supported_symbols_command
        )
        ...
        # معالج الصور - سيحلل أي صورة ترسل للبوت
        from telegram.ext import filters
        app.add_handler(MessageHandler(filters.PHOTO, handle_chart_image))
        
        print("✅ تم تحميل أوامر التداول المتقدم وتحليل الصور")
        
    except Exception as e:
        print(f"⚠️ خطأ في تحميل الأوامر المتقدمة: {e}")
    ...
    # بدء مراقبة التنبيهات والتداول التلقائي في الخلفية
    ...
    # تشغيل التداول التلقائي في thread منفصل
    trading_thread = threading.Thread(target=run_auto_trading, daemon=True)
    trading_thread.start()
    
    # تشغيل التداول المتقدم في thread منفصل
    advanced_thread = threading.Thread(target=run_advanced_trading, daemon=True)
